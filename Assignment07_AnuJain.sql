...
-- 2021-11-30, AnuJain, Created File
-- 2021-11-30, AnuJain, Responded to questions by writing SQL code
-- 2021-11-30, AnuJain, Completed File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_AnuJain')
	 Begin 
	  Alter Database [Assignment07DB_AnuJain] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_AnuJain;
	 End
	Create Database Assignment07DB_AnuJain;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_AnuJain;
...
Select 
vProducts.ProductName, 
CONCAT (DATENAME (month, InventoryDate),', ', Year (vInventories.InventoryDate)) AS InventoryDate,
vInventories.Count
...
from dbo.vProducts Left outer Join dbo.vInventories
on vProducts.ProductID = vInventories.ProductID
Order by vProducts.ProductName Asc, vInventories.InventoryDate Asc;
go
...
Select Top 10000 
vCategories.CategoryName,
CONCAT (DATENAME (month, InventoryDate),', ', Year (InventoryDate)) AS InventoryDate1,
